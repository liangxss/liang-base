        ...
        if (files != null)
        {
            for (int i = 0; i < files.length; i++)
            {
                ...
                if(fileKeys.length == files.length) {
                	...
                	//TODO 根据文件名设置contentType
                    builder.addPart(Headers.of("Content-Disposition", "form-data; name=\"" + fileKeys[0] + "\";filename=\"" + fileName + "\""),
                            RequestBody.create(MediaType.parse(guessMimeType(fileName)),file)
                    );
                }
...
//        for(Param param : params) {
//        	try {
//				obj.put(param.key, null == param.value ? "":param.value);
//			} catch (JSONException e) {
//				e.printStackTrace();
//			}
//        }
//        RequestBody requestBody = RequestBody.create(MediaType.parse("application/json; charset=utf-8"), obj.toString());
//      RequestBody requestBody = RequestBody.create(MediaType.parse("application/json; charset=utf-8"), "{\"token\":\"\",\"did\":\"12224\",\"deviceMark\":\"Android\"}");
